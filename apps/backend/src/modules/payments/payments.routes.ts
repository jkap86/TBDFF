...
import { setBuyInSchema, recordBuyInSchema, recordPayoutSchema } from './payments.schemas';
...
export function createPaymentRoutes(controller: PaymentController): Router {
  ...
  router.post('/:leagueId/payments/payouts', strictLimiter, validate(recordPayoutSchema), asyncHandler(controller.recordPayout));
